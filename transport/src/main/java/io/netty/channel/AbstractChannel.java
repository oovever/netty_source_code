...
// 给NioServerSocketChannel的eventLoop属性赋值eventLoop
            AbstractChannel.this.eventLoop = eventLoop;
// 如果当前线程与eventLoop绑定的为同一个线程 return thread == this.thread;
...
//                执行handler添加操作,回调执行注册前添加的handler；即init方法添加的handler将在这里被添加到pipeline并移除new initial方法
                pipeline.invokeHandlerAddedIfNeeded();

                safeSetSuccess(promise);
//               责任链调用channelRegister方法
...
//            判断channel是否被激活；绑定端口后才能执行channelActive操作
...
//                    write后会释放内存
        ...
        public final void  flush() {
            ...
            outboundBuffer.addFlush(); // 将outboundBuffer的数据flush到服务端
            flush0();
        }
