...
//            绑定端口到channel
        ...
        try {
//           连接远程地址;原生的connect是异步操作，connect立即连接返回true否则返回fasle
            boolean connected = SocketUtils.connect(javaChannel(), remoteAddress);
            if (!connected) {
//                连接失败注册OP_CONNEC事件，后续执行run方法时候再次连接
...
//        执行 Java 原生 NIO SocketChannel 关闭
    ...
    protected void doWrite(ChannelOutboundBuffer in) throws Exception {
        SocketChannel ch = javaChannel();
        int writeSpinCount = config().getWriteSpinCount(); //         int writeSpinCount = config().getWriteSpinCount(); // 只循环写 writeSpinCount 次,为了避免写大块儿数据时,阻塞其他线程过长时间
        do {
            if (in.isEmpty()) {
