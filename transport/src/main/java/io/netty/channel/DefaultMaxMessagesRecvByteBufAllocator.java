...
//       总连接数不能超过一定限制
//     respectMaybeMoreData默认为true，maybeMoreDataSupplier会根据此次是否满载而归来判断，如果此次byteBuf数据装满了就认为需要继续，否则不需要
        @Override
        public boolean continueReading(UncheckedBooleanSupplier maybeMoreDataSupplier) {
            return config.isAutoRead() &&
                   (!respectMaybeMoreData || maybeMoreDataSupplier.get()) &&
//                   maxMessagePerRead 读的次数是否小于16（默认值）  totalBytesRead 此次读的字节数>0
