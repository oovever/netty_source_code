...
//                      绑定端口
    ...
    final ChannelFuture initAndRegister() {
        ...
        try {
//          创建SocketChannel new NioServerSocketChannel()
...
//        config().group()= bossGroup
    ...
    // channel NioServerSocketChannel;channel.eventLoop() bossEventLoop
...
//       使用channel绑定的eventLoop的线程异步执行bind任务
        channel.eventLoop().execute(new Runnable() {
            @Override
            public void run() {
                if (regFuture.isSuccess()) { // channel注册成功后才去绑定端口
//                  调用NioServerSocketChannel进行bind
