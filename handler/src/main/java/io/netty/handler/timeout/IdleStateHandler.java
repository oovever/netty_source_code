...
//        childChannel注册后，调用pipeline addLast
...
//      触发用户自定义的userEvent userEventTriggered(ChannelHandlerContext ctx, Object evt)
...
//            读空闲时间-（当前时间-上次读取时间）=next_delay
            long nextDelay = readerIdleTimeNanos;
            if (!reading) { // 标记是否正在读，readComplete时会设置为false
                nextDelay -= ticksInNanos() - lastReadTime;
            }
//            next_delay<0 则代表已经超时了；重置delay超时时间，调度
            if (nextDelay <= 0) {
...
//                next_delay>0  代表此次心跳没有超时；next_delay作为超时时间重新调度
                // 1 2 3 4 5 6 7 8 9
//                queue task1 task2
//                        3s    2s+6s-1=7s
//                        5s 7s 8s  9s -1
...
//                 读
//                 3s
                // task task null read 5s
//                为什么心跳没有超时时，next_delay要作为下一次delay时间；考虑一种场景，第1s 读请求耗时3s，如果每次next_delay>0都按读超时时间(5s)重新调度的话，可能会在10s时发现读空闲，正常情况在8s时，就应该发现读空闲。3s到8这个时间间隔的读超时不会被校验到
