...
//    无效value的占位符，netty认为null是有效值，所以此处需要使用占位符标记无效值
...
//    获取InternalThreadLocalMap实例
    ...
    public static InternalThreadLocalMap get() {
        Thread thread = Thread.currentThread();
//        线程是FastThreadLocalThread通过fastGet获取否则通过slowGet获取
        if (thread instanceof FastThreadLocalThread) {
...
//    判断线程中InternalThreadLocalMap是否为空，为空则新建一个InternalThreadLocalMap并赋值
...
//       使用原生threadLocal方式get，没有命中索引需要线性探测
