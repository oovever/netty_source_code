...
//       创建任务执行队列
    ...
    protected SingleThreadEventExecutor(EventExecutorGroup parent, Executor executor,
            ...
            RejectedExecutionHandler rejectedHandler) {
        super(parent);
//      addTaskWakesUp代表添加task之后, NioEventLoop的select()操作是不是要唤醒 false表示需要唤醒
        this.addTaskWakesUp = addTaskWakesUp;
        this.maxPendingTasks = DEFAULT_MAX_PENDING_EXECUTOR_TASKS;
//       eventLoopGroup的executor
        /**
         ...
         *     }
         * }
         * 关联当前NioEventLoop与当前线程，每个NioEventLoop会关联一个单线程的Executor
         */
        this.executor = ThreadExecutorMap.apply(executor, this);
//      创建任务队列
...
//           执行任务队列中的任务
     ...
     *
     * @param maxDrainAttempts The maximum amount of times this method attempts to drain from queues. This is to
     *         prevent
...
//      executor绑定的线程是否与当前执行线程相同，添加空任务唤醒线程
        if (!inEventLoop) {
...
//      取消定时任务
    ...
    private void execute0(@Schedule Runnable task) {
        ObjectUtil.checkNotNull(task, "task");
//        非LazyRunnable 并且wakesUpForTask返回false，表示立即执行；默认返回true，如果需要修改可以继承SingleThreadEventExecutor重写该方法
...
//        immediate标识是否立即执行
//      判断当前线程与SingleThreadEventExecutor绑定的线程是否相同
        boolean inEventLoop = inEventLoop();
//        将任务添加到任务队列中
        addTask(task);
//        如果不相同,如果bossGroup只有一个线程，但是绑定多个端口；此处不会新建线程，跳过
        if (!inEventLoop) {
//          创建并启动一个线程
            startThread();
            if (isShutdown()) {
...
//       如果当前SingleThreadEventExecutor未启动
        if (state == ST_NOT_STARTED) {
//            设置状态为已启动，保证并发情况下只创建一个线程
...
//                    启动工作线程
    ...
    private void doStartThread() {
        ...
        executor.execute(new Runnable() {
            @Override
            public void run() {
//                绑定当前线程到SingleThreadEventExecutor,NioEventLoop和执行线程进行绑定
                ...
                try {
//                    执行run方法，循环从taskQueue队列中获取任务执行；匿名表达式的语法，作用是引用外部类的某个非static方法。https://blog.csdn.net/m0_45406092/article/details/117731756
